...
const commingsoon = (list, cost, fixed, game) => {
    let sum = 0;
    let listNum = JSON.parse(JSON.stringify(list));
    let costNum = JSON.parse(JSON.stringify(cost));
    ...
    console.log(listNum);
    let text = `당신은 ${piro}회 일찍 일어나서 걸어서 출근하면 ${piro}만큼 피곤해지고, 원장님에게 커피를 ${mium}회 얻어먹으면 ${mium}만큼 미움받지만 ${game.name}게임을 살수있게 됩니다!`;
    console.log(text);
};
